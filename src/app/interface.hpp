...
public:
    ...
    void plan(std::vector<brokkr::odin::PlanItem> plan, std::uint64_t total_flash_bytes);
...
private:
    struct TermSize { int rows = 0; int cols = 0; };
    struct Clip { std::string s; std::size_t w = 0; };
    ...
    std::uint64_t overall_done_ = 0, overall_total_ = 0, item_done_ = 0, item_total_ = 0;

    std::string notice_line_, status_line_;
    bool fatal_ = false;

    std::chrono::steady_clock::time_point start_{}, last_rate_ts_{}, last_redraw_{};
