        ...
        auto ci = [&](std::string_view needle) {
            for (std::size_t i = 0; i + needle.size() <= s.size(); ++i) {
                ...
                for (std::size_t j = 0; j < needle.size(); ++j) {
                    ...
                    if (static_cast<char>(std::tolower(a)) != static_cast<char>(std::tolower(b))) { ok = false; break; }
                }
                if (ok) return true;
            }
            return false;
...
void FlashInterface::plan(std::vector<brokkr::odin::PlanItem> plan, std::uint64_t total_flash_bytes) {
    std::lock_guard lk(mtx_);
    plan_ = std::move(plan);
...
void FlashInterface::done_item(std::size_t index) {
    std::lock_guard lk(mtx_);
    if (index < plan_done_.size()) plan_done_[index] = true;
    redraw_(true);
}
...
std::string FlashInterface::bytes_h_(std::uint64_t b) {
    ...
    std::ostringstream oss;
    if (!i) oss << static_cast<std::uint64_t>(v) << u[i];
    else oss << std::fixed << std::setprecision(v >= 10 ? 1 : 2) << v << u[i];
    return oss.str();
}
...
void FlashInterface::redraw_(bool force) {
    ...
    if (!tty_) {
        if (!force) return;
        std::cout << "Devices=" << dev_count_
                  << " Stage=" << (stage_.empty() ? "-" : stage_)
                  << " Overall=" << bytes_h_(overall_done_) << "/" << bytes_h_(overall_total_)
                  << " cpu_bl_id=" << (model_.empty() ? "-" : model_)
    ...
    {
        std::string title = "Brokkr Flash v" + brokkr::app::version_string() + " --- Copyright (c) 2026 Gabriel2392.";
        if (color_) out << kBold;
        emit(kGray, title);
        if (color_) out << kReset;
    }

    emit(kBlue, (static_cast<std::ostringstream&&>(std::ostringstream{}
        << "Devices: " << dev_count_ << "  IDs: " << join_ids(dev_ids_)
        << "  cpu_bl_id: " << (model_.empty() ? "-" : model_))).str());
    ...
    {
        const std::string prefix = (static_cast<std::ostringstream&&>(std::ostringstream{} << std::setw(3) << overall_pct << "% ")).str();
        const std::string bytes = "  " + bytes_h_(overall_done_) + "/" + bytes_h_(overall_total_);
        const std::string rate  = "  " + rate_h_(ema_rate_bps_);
        const std::string etas  = "  ETA " + eta_h_(eta);

        auto suf = [&](bool r, bool e) { std::string s = bytes; if (r) s += rate; if (e) s += etas; return s; };
        ...
        if (!layout(suf(true, true)) && !layout(suf(false, true)) && !layout(suf(false, false))) { used = bytes; bar_w = 10; }
    ...
    emit(kCyan, (static_cast<std::ostringstream&&>(std::ostringstream{} << "Plan (" << plan_.size() << " items)")).str());
    ...
    if (first) emit(kGray, (static_cast<std::ostringstream&&>(std::ostringstream{} << "↑ " << first << " hidden")).str());
    ...
    for (std::size_t i = first; i < last; ++i) {
        ...
        if (it.kind == brokkr::odin::PlanItem::Kind::Part) { id = std::to_string(it.part_id); pit = it.pit_file_name; }

        emit(col, row(st, id, it.part_name, pit, bytes_h_(it.size), it.source_base));
    }

    if (plan_.size() > last) emit(kGray, (static_cast<std::ostringstream&&>(std::ostringstream{} << "↓ " << (plan_.size() - last) << " hidden")).str());
