...
constexpr std::array<std::byte, 4> kLz4Magic{
...
static void read_exact(ByteSource& src, std::span<std::byte> out) {
    ...
    while (off < out.size()) {
        ...
        if (got == 0) throw std::runtime_error("LZ4: unexpected EOF");
        off += got;
    }
}
...
Lz4FrameHeaderInfo parse_lz4_frame_header_or_throw(ByteSource& src) {
    ...
    if (version != 1) throw std::runtime_error("LZ4: unsupported frame version");
    ...
    if (!info.block_independence) throw std::runtime_error("LZ4: frame must use independent blocks");
    if (info.block_checksum) throw std::runtime_error("LZ4: block checksum not supported");
    if (info.has_dict_id) throw std::runtime_error("LZ4: dictionary ID not supported");
    if (!info.has_content_size) throw std::runtime_error("LZ4: content size missing (compress with --content-size)");

    info.max_block_size = max_block_size_from_bd(info.bd);
    if (info.max_block_size == 0) throw std::runtime_error("LZ4: invalid BD/max block size");
    ...
    if (info.content_size > LZ4_ONE_MIB && info.max_block_size != static_cast<std::size_t>(LZ4_ONE_MIB)) {
        throw std::runtime_error("LZ4: content > 1MiB requires 1MiB blocks (compress with -B6)");
    }
...
Lz4BlockStreamReader::Lz4BlockStreamReader(std::unique_ptr<ByteSource> src)
    : src_(std::move(src))
{
    if (!src_) throw std::runtime_error("LZ4: null source");
...
std::size_t Lz4BlockStreamReader::read_n_blocks(std::size_t n, std::vector<std::byte>& out) {
    if (n == 0) return 0;

    const std::size_t before = out.size();
    const std::size_t total = total_blocks_1m();
    if (blocks_read_ + n > total) throw std::runtime_error("LZ4: too many blocks requested");
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        if (raw_sz == 0) throw std::runtime_error("LZ4: encountered endmark unexpectedly");
...
bool Lz4DecompressedSource::fill_next_block_() {
    if (produced_ >= total_out_) return false;
    ...
    read_exact_({szb.data(), szb.size()});
    const std::uint32_t raw_sz = u32_le(std::span<const std::byte, 4>(szb.data(), 4));
    if (raw_sz == 0) throw std::runtime_error("LZ4: encountered endmark unexpectedly while decoding");
    ...
    if (uncompressed) {
        ...
        if (ret < 0) throw std::runtime_error("LZ4: decompression failed (LZ4_decompress_safe)");
