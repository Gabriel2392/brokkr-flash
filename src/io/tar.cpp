...
inline std::uint64_t parse_u64_dec(std::string_view s) {
    ...
    if (ec != std::errc{}) throw std::runtime_error("PAX: invalid decimal number");
    if (ptr != s.data() + s.size()) throw std::runtime_error("PAX: trailing junk in decimal number");
    return v;
}
...
std::uint64_t TarArchive::parse_octal(std::string_view s) {
    while (!s.empty() && (s.front() == ' ' || s.front() == '\t' || s.front() == '\0'))
        s.remove_prefix(1);
...
std::uint64_t TarArchive::parse_tar_number(const char* p, std::size_t n) {
    ...
    if (b0 & 0x80) {
        ...
        if (negative) throw std::runtime_error("Tar: negative base-256 numeric field");

        std::uint64_t val = static_cast<std::uint64_t>(b0 & 0x3Fu);
        for (std::size_t i = 1; i < n; ++i) {
            if (val > (std::numeric_limits<std::uint64_t>::max() >> 8)) {
                throw std::runtime_error("Tar: base-256 numeric field too large for uint64");
            }
...
bool TarArchive::validate_header_checksum(std::span<const std::byte, 512> header) {
...
void TarArchive::scan_() {
    std::ifstream in(path_, std::ios::binary);
    if (!in.is_open()) {
        throw std::runtime_error("TarArchive: cannot open: " + path_);
    }
    ...
    auto read_exact = [&](std::byte* dst, std::size_t n) {
        ...
        if (!in.good()) throw std::runtime_error("TarArchive: short read: " + path_);
        pos += n;
    };
    ...
    auto skip_exact = [&](std::uint64_t n) {
        ...
        if (n > static_cast<std::uint64_t>(std::numeric_limits<std::streamoff>::max())) {
            throw std::runtime_error("TarArchive: entry too large for seekg");
        }
        in.seekg(static_cast<std::streamoff>(n), std::ios::cur);
        if (!in.good()) throw std::runtime_error("TarArchive: seek failed: " + path_);
        pos += n;
    };
    ...
    for (;;) {
        ...
        if (validate_ && !validate_header_checksum(std::span<const std::byte, 512>(header))) {
            throw std::runtime_error("TarArchive: invalid header checksum in: " + path_);
        }
        ...
        if (typeflag == 'x' || typeflag == 'g') {
            if (size > (1024ull * 1024ull * 8ull)) {
                throw std::runtime_error("TarArchive: refusing huge PAX header");
            }

            std::string payload;
            payload.resize(static_cast<std::size_t>(size));
            if (size) read_exact(reinterpret_cast<std::byte*>(payload.data()), static_cast<std::size_t>(size));
            skip_exact(round_up_512(size) - size);

            PaxKV kv = parse_pax_payload(payload);
            if (typeflag == 'g') pax_global.merge_from(kv);
            else                 pax_next.merge_from(kv);
            continue;
        }

        if (typeflag == 'L') {
            if (size > (1024ull * 1024ull * 8ull)) {
                throw std::runtime_error("TarArchive: refusing huge GNU longname header");
            }
