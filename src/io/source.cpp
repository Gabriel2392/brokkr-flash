...
public:
  explicit RawFileSource(std::filesystem::path p)
    ...
    if (!in_.is_open())
      throw std::runtime_error("open_raw_file: cannot open");

    std::error_code ec;
    const auto sz = std::filesystem::file_size(path_, ec);
    if (ec)
      throw std::runtime_error("open_raw_file: file_size failed");
    size_ = static_cast<std::uint64_t>(sz);

    in_.seekg(0, std::ios::beg);
    if (!in_.good())
      throw std::runtime_error("open_raw_file: seek failed");
  }
...
public:
  TarEntrySource(std::filesystem::path tar, TarEntry e)
      : tar_path_(std::move(tar)), entry_(std::move(e)),
        in_(tar_path_, std::ios::binary), remaining_(entry_.size) {
    if (!in_.is_open())
      throw std::runtime_error("open_tar_entry: cannot open tar");

    if (entry_.data_offset > static_cast<std::uint64_t>(
                                 std::numeric_limits<std::streamoff>::max())) {
      throw std::runtime_error(
          "open_tar_entry: data_offset too large for seekg");
    }

    in_.seekg(static_cast<std::streamoff>(entry_.data_offset), std::ios::beg);
    if (!in_.good())
      throw std::runtime_error("open_tar_entry: seek failed");
  }
