...
uint16_t extract_hex(const std::string& str, const std::string& key) {
    ...
    for (char& c : lower_str) c = static_cast<char>(std::tolower(c));

    size_t pos = lower_str.find(key);
    if (pos == std::string::npos) return 0;

    pos += key.length();
    if (pos + 4 > lower_str.length()) return 0;

    try {
        return static_cast<uint16_t>(std::stoul(lower_str.substr(pos, 4), nullptr, 16));
    }
    catch (...) {
        return 0;
    }
...
std::vector<UsbDeviceInfo> enumerate_usb_devices_windows(uint16_t target_vid, const std::vector<uint16_t>& allowed_pids) {
    std::vector<UsbDeviceInfo> result;

    // Ask SetupAPI to look at ALL devices currently present on the system.
    // We must check everything because Samsung CDC devices can hide under both "Ports" and "Modems".
    ...
    for (DWORD i = 0; SetupDiEnumDeviceInfo(hDevInfo, i, &devInfoData); ++i) {
        ...
        if (SetupDiGetDeviceRegistryPropertyA(hDevInfo, &devInfoData, SPDRP_HARDWAREID, nullptr, (PBYTE)hwId, sizeof(hwId), nullptr)) {
            ...
            bool match_pid = allowed_pids.empty() || (std::find(allowed_pids.begin(), allowed_pids.end(), pid) != allowed_pids.end());

            if (match_vid && match_pid) {
                // 3. We found a matching USB device! Now, open its registry key to find the COM port name.
                ...
                if (hKey != INVALID_HANDLE_VALUE) {
                    ...
                    // Modems (like Samsung Download Mode) sometimes store it in "AttachedTo"
                    if (!found_port) {
                    ...
                    if (found_port) {
                        ...
                        std::cout << "Found Device: " << info.device_path << " (VID: 0x"
                            << std::hex << vid << ", PID: 0x" << pid << std::dec << ")\n";
...
std::vector<UsbDeviceSysfsInfo> enumerate_usb_devices_sysfs(const EnumerateFilter& filter) {
    ...
    for (const auto& dev : devices) {
        ...
        // Windows does not have a direct equivalent of busnum/devnum, so we can leave them as -1 or parse from the device path if needed.
...
std::optional<UsbDeviceSysfsInfo> find_by_sysname(std::string_view sysname) {
    auto devices = enumerate_usb_devices_sysfs(EnumerateFilter{.vendor=0, .products={}}); // Get all devices
    for (const auto& dev : devices) {
