...
UsbFsDevice::UsbFsDevice(std::string devnode)
    : devnode_(std::move(devnode)) {
}

UsbFsDevice::~UsbFsDevice() {
    close();
}
...
void UsbFsDevice::open_and_init() {
    ...
    if (port_path.find("\\\\.\\") == std::string::npos && port_path.find("COM") != std::string::npos) {
        port_path = "\\\\.\\" + port_path;
    }

	std::cout << "Opening COM port: " << port_path << "\n";
    ...
    if (handle_ == INVALID_HANDLE_VALUE) {
        throw std::system_error(GetLastError(), std::system_category(), "Failed to open COM port: " + port_path);
    }
    ...
    if (GetCommState(handle_, &dcbSerialParams)) {
        ...
        if (!SetCommState(handle_, &dcbSerialParams)) {
            close();
            throw std::system_error(GetLastError(), std::system_category(), "Failed to set COM state");
        }
    }
    else {
        close();
        throw std::system_error(GetLastError(), std::system_category(), "Failed to get COM state");
    }

    // Note: A COM port is a stream, not a packet interface. 
    // We leave the UsbEndpoints (eps_) zeroed out as they are irrelevant for Serial IO.
}
