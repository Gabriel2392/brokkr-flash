...
PitTable parse(std::span<const std::byte> bytes) {
  if (bytes.size() < sizeof(PitHeaderWire)) {
    throw std::runtime_error("PIT parse: buffer too small for header");
  }
  ...
  if (hdr.magic != PIT_MAGIC) {
    throw std::runtime_error("PIT parse: wrong magic");
  }
  if (hdr.count < 0) {
    throw std::runtime_error("PIT parse: negative partition count");
  }

  const std::size_t count = static_cast<std::size_t>(hdr.count);
  const std::size_t required =
      sizeof(PitHeaderWire) + count * sizeof(PartitionInfoWire);
  if (bytes.size() < required) {
    throw std::runtime_error(
        "PIT parse: buffer smaller than declared partition table");
  }
  ...
  std::int32_t max_blockSize = 0;
  std::int32_t max_offset = 0;
  std::int32_t max_blockLength = 0;
  ...
  const bool blockSize_is_begin_block =
      (max_blockSize > 4096) && (max_offset <= 4096);
  ...
  for (std::size_t i = 0; i < out.partitions.size(); ++i) {
    by_dev[out.partitions[i].dev_type].push_back(i);
  }
  ...
  for (auto &[dev, idxs] : by_dev) {
    ...
    for (std::size_t k = 0; k < idxs.size(); ++k) {
      ...
      const std::uint64_t bb = (p.block_bytes > 0)
                                   ? static_cast<std::uint64_t>(p.block_bytes)
                                   : 0ull;
      const std::uint64_t bc =
          (p.block_size > 0) ? static_cast<std::uint64_t>(p.block_size) : 0ull;
      p.file_size = bb * bc;
    }
  }
...
const Partition *
PitTable::find_by_file_name(std::string_view basename) const noexcept {
  if (basename.empty())
    return nullptr;

  auto it = std::ranges::find_if(
      partitions, [&](const Partition &p) { return p.file_name == basename; });
...
std::optional<std::int32_t> PitTable::common_block_size() const noexcept {
  ...
  if (std::ranges::all_of(partitions, [&](const Partition &p) {
        return p.block_bytes == bs;
      })) {
    return bs;
  }
  return std::nullopt;
