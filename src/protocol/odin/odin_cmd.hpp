...
struct OdinError : std::runtime_error { using std::runtime_error::runtime_error; };
struct OdinIoError : OdinError { using OdinError::OdinError; };
struct OdinProtocolError : OdinError { using OdinError::OdinError; };
...
public:
    ...
    void send_total_size(std::uint64_t total_size, ProtocolVersion proto, unsigned retries = 8);
    ...
    void end_download(std::int32_t size_to_flash,
                      std::int32_t part_id,
    ...
    ResponseBox recv_checked_response(std::int32_t expected_id, std::int32_t* out_ack = nullptr, unsigned retries = 8);
    void send_request(const RequestBox& rq, unsigned retries = 8);

private:
    ResponseBox rpc_(RqtCommandType type,
