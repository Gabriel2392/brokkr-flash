...
inline void require_connected(brokkr::core::IByteTransport &c) {
  if (!c.connected())
    throw OdinIoError("transport not connected");
}

inline void check_resp(std::int32_t expected_id, const ResponseBox &r,
                       std::int32_t *out_ack) {
  if (r.id == BOOTLOADER_FAIL)
    throw OdinProtocolError("Bootloader returned FAIL");
  if (r.id == std::numeric_limits<std::int32_t>::min())
    throw OdinProtocolError("Invalid response id (INT_MIN)");
  if (r.id != expected_id)
    throw OdinProtocolError("Unexpected response id");
  if (out_ack)
    *out_ack = r.ack;
  else if (r.ack < 0)
    throw OdinProtocolError("Operation failed (negative ack)");
}
...
static std::int32_t hi32(std::uint64_t v) {
  return static_cast<std::int32_t>(
      static_cast<std::uint32_t>((v >> 32) & 0xFFFFFFFFull));
}
...
static std::int32_t require_i32_total(std::uint64_t v) {
  ...
  if (v > max)
    throw OdinProtocolError(
        "TOTALSIZE exceeds ODIN int32 limit on protocol v0/v1");
...
void OdinCommands::recv_raw(std::span<std::byte> data, unsigned retries) {
  ...
  while (off < data.size()) {
    ...
    if (got <= 0)
      throw OdinIoError("receive failed");
...
ResponseBox OdinCommands::recv_checked_response(std::int32_t expected_id,
                                                std::int32_t *out_ack,
                                                unsigned retries) {
  ResponseBox r{};
...
ResponseBox OdinCommands::rpc_(RqtCommandType type, RqtCommandParam param,
  ...
  return recv_checked_response(static_cast<std::int32_t>(type), out_ack,
                               retries);
}
...
void OdinCommands::handshake(unsigned retries) {
  ...
  if (conn_.kind() == brokkr::core::IByteTransport::Kind::UsbBulk) {
    static constexpr std::array<std::byte, 5> ping{
  ...
  while (have < expected.size()) {
    const int got = conn_.recv(brokkr::core::u8(std::span<std::byte>(
                                   resp.data() + have, resp.size() - have)),
                               retries);
    if (got <= 0)
      throw OdinIoError("Handshake receive failed");
...
InitTargetInfo OdinCommands::get_version(unsigned retries) {
  const std::int32_t ints[] = {
      static_cast<std::int32_t>(ProtocolVersion::PROTOCOL_VER5)};

  std::int32_t ack_i32 = 0;
  (void)rpc_(RqtCommandType::RQT_INIT, RqtCommandParam::RQT_INIT_TARGET, ints,
             {}, &ack_i32, retries);
...
void OdinCommands::setup_transfer_options(std::int32_t packet_size,
                                          unsigned retries) {
  const std::int32_t ints[] = {packet_size};
  (void)rpc_(RqtCommandType::RQT_INIT, RqtCommandParam::RQT_INIT_PACKETSIZE,
             ints, {}, nullptr, retries);
}

void OdinCommands::send_total_size(std::uint64_t total_size,
                                   ProtocolVersion proto, unsigned retries) {
  if (proto <= ProtocolVersion::PROTOCOL_VER1) {
    const std::int32_t v = require_i32_total(total_size);
    const std::int32_t ints[] = {v};
    (void)rpc_(RqtCommandType::RQT_INIT, RqtCommandParam::RQT_INIT_TOTALSIZE,
               ints, {}, nullptr, retries);
  } else {
...
std::int32_t OdinCommands::get_pit_size(unsigned retries) {
  ...
  (void)rpc_(RqtCommandType::RQT_PIT, RqtCommandParam::RQT_PIT_GET, {}, {},
             &pitSize, retries);
  return pitSize;
}
...
void OdinCommands::get_pit(std::span<std::byte> out, unsigned retries) {
  ...
  if (out.empty())
    throw OdinProtocolError("PIT output buffer empty");
  ...
  for (std::size_t idx = 0; idx < parts; ++idx) {
    ...
    const std::size_t sizeToDownload = std::min<std::size_t>(
        PIT_TRANSMIT_UNIT, pitSize - (PIT_TRANSMIT_UNIT * idx));
    const std::size_t off = idx * PIT_TRANSMIT_UNIT;
    recv_raw(out.subspan(off, sizeToDownload), retries);
  }

  (void)conn_.recv_zlp();
  (void)rpc_(RqtCommandType::RQT_PIT, RqtCommandParam::RQT_PIT_COMPLETE, {}, {},
             nullptr, retries);
}

void OdinCommands::set_pit(std::span<const std::byte> pit, unsigned retries) {
  if (pit.empty())
    throw OdinProtocolError("PIT buffer empty");
  if (pit.size() >
      static_cast<std::size_t>(std::numeric_limits<std::int32_t>::max())) {
    throw OdinProtocolError("PIT too large for ODIN int32");
  }

  (void)rpc_(RqtCommandType::RQT_PIT, RqtCommandParam::RQT_PIT_SET, {}, {},
             nullptr, retries);

  const auto pitSize32 = static_cast<std::int32_t>(pit.size());
  (void)rpc_(RqtCommandType::RQT_PIT, RqtCommandParam::RQT_PIT_START,
...
void OdinCommands::begin_download(std::int32_t rounded_total_size,
                                  unsigned retries) {
  (void)rpc_(RqtCommandType::RQT_XMIT, RqtCommandParam::RQT_XMIT_DOWNLOAD, {},
             {}, nullptr, retries);
  (void)rpc_(RqtCommandType::RQT_XMIT, RqtCommandParam::RQT_XMIT_START,
             std::span{&rounded_total_size, 1}, {}, nullptr, retries);
}

void OdinCommands::begin_download_compressed(std::int32_t comp_size,
                                             unsigned retries) {
  (void)rpc_(RqtCommandType::RQT_XMIT,
             RqtCommandParam::RQT_XMIT_COMPRESSED_DOWNLOAD, {}, {}, nullptr,
             retries);
  (void)rpc_(RqtCommandType::RQT_XMIT,
...
void OdinCommands::end_download_impl_(RqtCommandParam complete_param,
  ...
  (void)rpc_(RqtCommandType::RQT_XMIT, complete_param, data, {}, nullptr,
             retries);
}

void OdinCommands::end_download(std::int32_t size_to_flash,
                                std::int32_t part_id, std::int32_t dev_type,
                                bool is_last, std::int32_t bin_type,
                                bool efs_clear, bool boot_update,
                                unsigned retries) {
  end_download_impl_(RqtCommandParam::RQT_XMIT_COMPLETE, size_to_flash, part_id,
                     dev_type, is_last, bin_type, efs_clear, boot_update,
                     retries);
}
...
void OdinCommands::end_download_compressed(std::int32_t decomp_size_to_flash,
                                           ...
                                           bool efs_clear, bool boot_update,
                                           unsigned retries) {
  end_download_impl_(RqtCommandParam::RQT_XMIT_COMPRESSED_COMPLETE,
                     decomp_size_to_flash, part_id, dev_type, is_last, bin_type,
...
void OdinCommands::shutdown(ShutdownMode mode, unsigned retries) {
  ...
  auto close_cmd = [&](RqtCommandParam p) {
    (void)rpc_(RqtCommandType::RQT_CLOSE, p, {}, {}, nullptr, retries);
  };

  if (mode == ShutdownMode::NoReboot) {
    close_cmd(RqtCommandParam::RQT_CLOSE_END);
    return;
  }
  if (mode == ShutdownMode::Reboot) {
    close_cmd(RqtCommandParam::RQT_CLOSE_END);
    close_cmd(RqtCommandParam::RQT_CLOSE_REBOOT);
    return;
  }
  ...
  for (std::size_t i = 0; i < kAutoTest.size(); ++i)
    msg[i] = static_cast<std::byte>(kAutoTest[i]);
