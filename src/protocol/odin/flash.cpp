...
static std::string read_text_or_throw(io::ByteSource& src, std::size_t max_bytes, std::string_view what) {
    const std::uint64_t n64 = src.size();
    if (n64 > max_bytes) throw std::runtime_error(std::string(what) + " too large");
    const std::size_t n = static_cast<std::size_t>(n64);

    std::string s(n, '\0');
    for (std::size_t off = 0; off < n;) {
        const std::size_t got = src.read(std::as_writable_bytes(std::span<char>(s.data() + off, n - off)));
        if (!got) throw std::runtime_error("Short read: " + std::string(what));
        off += got;
    }
    return s;
}

static std::vector<std::string> parse_download_list_or_throw(std::string_view txt) {
    ...
    for (std::size_t pos = 0; pos <= txt.size();) {
        ...
        if (!seen.insert(name).second) throw std::runtime_error("download-list.txt contains duplicate entry: " + name);
        names.push_back(std::move(name));
    }

    if (names.empty()) throw std::runtime_error("download-list.txt is empty");
    return names;
}
...
static std::optional<io::TarEntry> find_download_list_entry(const io::TarArchive& tar) {
    for (const auto& e : tar.entries()) if (is_download_list_name(e.name)) return e;
    return std::nullopt;
}
...
static ImageSpec make_spec(ImageSpec::Kind kind,
...
std::unique_ptr<io::ByteSource> ImageSpec::open() const {
    ...
    throw std::runtime_error("ImageSpec::open: unknown kind");
}
...
std::vector<ImageSpec> expand_inputs_tar_or_raw(const std::vector<std::filesystem::path>& inputs) {
    ...
    for (const auto& p : inputs) {
        ...
        if (auto e = find_download_list_entry(tar)) {
            ...
            auto names = parse_download_list_or_throw(read_text_or_throw(*src, 128 * 1024, "download-list.txt"));
            if (!dl) dl = std::move(names);
            else if (!lists_equal(*dl, names)) throw std::runtime_error("Multiple download-list.txt files found with different contents");
        }
    }
    ...
    if (dl) {
        ...
        for (const auto& p : inputs) {
            if (io::TarArchive::is_tar_file(p.string())) {
                ...
                for (const auto& e : tar.entries()) {
                    ...
                    merge(cands, SourceCandidate{
                        .kind = ImageSpec::Kind::TarEntry,
                        .path = p,
                        .entry = e,
                        .basename = base,
                        .source_basename = sb,
        ...
        for (const auto& name : *dl) {
            ...
            if (it == cands.end()) throw std::runtime_error("download-list.txt references missing file: " + name);
    ...
    for (const auto& p : inputs) {
        if (io::TarArchive::is_tar_file(p.string())) {
            ...
            for (const auto& e : tar.entries()) {
                ...
                out.push_back(make_spec(ImageSpec::Kind::TarEntry,
                                        p,
                                        e,
                                        p.string() + ":" + e.name,
                                        io::basename(e.name),
        ...
        auto src = io::open_raw_file(p);
        out.push_back(make_spec(ImageSpec::Kind::RawFile,
                                p,
                                {},
                                p.string(),
...
std::vector<FlashItem> map_to_pit(const pit::PitTable& pit_table, const std::vector<ImageSpec>& sources) {
    ...
    for (const auto& s : sources) {
        ...
        if (inserted) items.push_back(FlashItem{.part = *part, .spec = s});
        else items[it->second] = FlashItem{.part = *part, .spec = s};
    }

    if (items.empty()) throw std::runtime_error("No flashable items after PIT mapping");
    return items;
}
                                     ...
                                     std::vector<std::byte>& stream)
{
    ...
    PreparedLz4Window w{};
    if (rem > static_cast<std::uint64_t>(max_blocks) * kOneMiB) { w.decomp_size = static_cast<std::uint64_t>(max_blocks) * kOneMiB; w.last = false; }
    else { w.decomp_size = rem; w.last = true; }

    const std::size_t blocks = !w.last ? static_cast<std::size_t>(w.decomp_size / kOneMiB) : r.blocks_remaining_1m();
